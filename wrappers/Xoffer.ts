...
export type XofferConfig = {
    total_supply: number; admin: Address; content: Cell; wallet_code: Cell
};
export type JettonMinterContent = {
    type: 0 | 1,
    uri: string
};
export function xofferConfigToCell(config: XofferConfig): Cell {
    return beginCell().storeCoins(config.total_supply)
...
export class Xoffer implements Contract {
    constructor(readonly address: Address, readonly init?: { code: Cell; data: Cell }) { }

    static createFromAddress(address: Address) {
        return new Xoffer(address);
    }

    static createFromConfig(config: XofferConfig, code: Cell, workchain = 0) {
        const data = xofferConfigToCell(config);
        const init = { code, data };
        return new Xoffer(contractAddress(workchain, init), init);
    }
    ...
    async sendMint(provider: ContractProvider, via: Sender, to: Address, jetton_amount: bigint, forward_ton_amount: bigint, total_ton_amount: bigint) {
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Xoffer.mintMessage(this.address, to, jetton_amount, forward_ton_amount, total_ton_amount),
    ...
    */
    ...
    async sendDiscovery(provider: ContractProvider, via: Sender, owner: Address, include_address: boolean, value:bigint = toNano('0.1')) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Xoffer.discoveryMessage(owner, include_address),
            value: value,
        });
    }
    ...
    async sendChangeAdmin(provider: ContractProvider, via: Sender, newOwner: Address) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Xoffer.changeAdminMessage(newOwner),
            value: toNano("0.05"),
        });
    }
    ...
    async sendChangeContent(provider: ContractProvider, via: Sender, content: Cell) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Xoffer.changeContentMessage(content),
            value: toNano("0.05"),
        });
    }
